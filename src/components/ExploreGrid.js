...
import { Grid, Box, Typography, SvgIcon, Slide } from "@mui/material";
...
function ExploreGrid(props) {
  ...
  return (
    <>
      <Slide direction="down" in={props.bol} mountOnEnter unmountOnExit>
        <Box p={2} bgcolor="#F1F6F4" width={1}>
            ...
            <Grid container spacing={2}>
              <Grid item xs={4}>
                <Box
                  onClick={() => {
                    if (prev.length == 0) {
                      props.onClick("Entrepreneurship");
                      prev = "";
                    } else {
                      props.onClick("");
                      prev = "Entrepreneurship";
                    }
                  }}
